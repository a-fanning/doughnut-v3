...
grpSoc3 <- left_join(grpSoc2, sumAbs, by=c("domain", "dimension", "type", "indicator", "indCode", "date")) %>%
  ...
  ungroup() %>%
  mutate(group = factor(group, levels=c("Top-20", "Middle-40", "Bottom-40")))
...
socShares <- ggplot(socChart, aes(x=axisLab, y=share, fill=group)) +
  ...
  coord_flip() +
  scale_fill_manual(name="Country cluster", labels=c("richest 20%", "middle 40%", "poorest 40%"), values=cols) +
...
#ggsave("./figures/11_grpShares_R2.png", width = 183, height = 180, units = "mm", device = "png")
