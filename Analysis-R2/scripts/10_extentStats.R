...
ecoSoc3 <- read_csv("./myData/9_20250113_Doughnut-GlobalTablesData.csv") %>%
  mutate(dimension = factor(dimension, levels = c("climate change", "ocean acidification", "chemical pollution",
	"nutrient pollution", "air pollution", "freshwater disruption", "land conversion", "biodiversity breakdown",
	"ozone depletion", "food", "health", "education", "income and work", "water", "energy", "connectivity",
	"housing", "equality", "social cohesion", "political voice", "peace and justice")))
...
#save figure to file
#ggsave("./figures/10_shortfallRange-by-year_Boxplots_R2.png", width = 180, height = 80, units = "mm", device="png")
...
#ggsave("./figures/10_overshootRange-by-year_Boxplots_R2.png", width = 180, height = 80, units = "mm", device="png")
...
eco %>% 
  filter(!indicator %in% c("blueDev", "extinction1900", "interhemAOD")) %>%
  summary() #Start:[52,133%]  End:[89,227%]
...
#------------------------------------------------------------------------------------------
soc %>% 
    filter(!indicator %in% c("publicTrans","racialEquality"), betterWorse == "improving" , pValSlope < 0.01, slope < -0.02) 
...
soc %>% 
  ...
  ungroup() %>% filter(ambition > 2 & ambition <= 9)

soc %>% 
  filter(!indicator %in% c("publicTrans","racialEquality"), pValSlope > 0.01) # 
  rowwise() %>%
  #mutate(ambition = (slopeD - slope)/ slope) %>%
  mutate(ambition = slopeD / slope) %>%
  ungroup() %>% filter(ambition > 2 & ambition <= 9)
