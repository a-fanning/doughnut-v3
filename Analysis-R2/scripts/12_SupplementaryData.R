...
allYears1 <- left_join(allYears, units, by=c("domain", "dimension", "indCode", "indicator")) %>%
  ...
  mutate(indicator = ifelse(indCode == "EQ1", "genderInequalityIndex", indicator),
    ratio = ifelse(domain == "ecological", ratio-1,
      ifelse(domain == "social", 1-ratio, "ERROR"))) %>%
  ungroup() %>%
  arrange(domain, dimension, indCode)

#write to file
#write_csv(allYears1, "./myData/12_20250115_globalDoughnutData_2000-2022.csv")
...
# write to file
#write_csv(regData, "./myData/12_20250115_globalDoughnutData_regressions.csv")
...
#write to file
#write_csv(grpDat2, "./myData/12_20250115_groupDoughnutData_2017.csv")
