...
#------------------------------------------------------------------------
#		MY FUNCTIONS TO ESTIMATE DYNAMIC STATISTICAL FORECASTS
#------------------------------------------------------------------------
...
#------------------------------------------------------------------------
#	MY FUNCTIONS TO EXTRACT AND SPREAD VALUES FOR NESTING
#------------------------------------------------------------------------
...
spreadRatio <- function(dat, ...){
  args <- rlang::enexprs(...)
  dat %>% filter(!!! args) %>%
  select(place, placeCode, date, ratio) %>%
  spread(date, ratio)
}

#make blank tibbles to fill series1 or series2 (NOTE: "WORLD"/"WLD" ARE HARD-CODED/NOT SCALABLE)
#(depending on if there are 1 or 2 indicators per dimension)
blankRatio <- function(){
  tibble(place="World", placeCode="WLD", date=2000:2030, val=0) %>%
  spread(date, val)
}

#loop to pull out variable values and put into a list
pullVars <- function(dat,index){
  dat <- dat %>% select(-place, -placeCode)
  for(j in 1:ncol(dat)){
    myList[j] <- list(
    dat[[j]])
  }
